...
按匿名画像（主厨 1700／最高 1780／眼高 1600／屏高 600）核对本册个性化落点，四条对上公式：
台面高 900–950（主厨/2+[50,100]）、挂杆高 2136（最高×1.2）、镜面中心 1600（眼高直取）、
电视观看距离 1800–2400（屏高×[3,4]）——**数字全部求值线算得，写手只写理由文字**。
**小案**：`lkp-tv-distance` 种子无 `unit` → 正文渲出"1800–2400"裸数无量纲（渲染层 unit=None
合法略过，但客户读不出单位）。上游一行修（种子补 unit: mm），归主线；量纲类落点 unit 是否应
必填，可并入 verify_seeds 校验。
